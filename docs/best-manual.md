...
ZEROは、YouTube動画の「企画立案」から「台本作成」までを一気通貫でサポートするAIエージェントです。
...
> ※ 初回アクセス時にベーシック認証が求められます。別途案内のユーザー名とパスワードを入力してください。
...
どの機能を使うべきか迷ったら、このフローチャートを参考にしてください。

```mermaid
graph TD
    A[動画を作りたい] --> B{現在の状態は？}
    B -->|アイデアは<br>あるが曖昧| C[企画案作成AI]
    B -->|企画は決定<br>冒頭を強化したい| D[冒頭台本作成AI]
    B -->|ゼロから<br>台本全文が必要| E[台本全文作成AI]
    B -->|長尺台本から<br>ショート版作成| F[ショート台本作成AI]
```
...
AIがあなたの特徴を理解し、パーソナライズされた台本を作成するためです。
...
> 💡 **ポイント**: 詳細に入力するほど精度UP。ただし時間をかけすぎないこと（5分以内が目安）。
...
「ログインできない」「出力が途中で止まった」などの不具合は、Discordの��ポートチャンネルでお問い合わせください。

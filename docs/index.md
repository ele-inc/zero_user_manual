---
layout: default
title: ''
mermaid: true
---

# **える式台本生成AIエージェント ZERO 活用ガイド**

執筆：担当エンジニアSHUN

---

## **本ドキュメントの概要**

「える式台本生成AIエージェント ZERO」の利用方法をまとめたドキュメントです。

利用開始前に必ずご一読ください。

**ツールURL:** [https://youtube-content-gen.vercel.app/](https://youtube-content-gen.vercel.app/)

**⚠️ 初回アクセス時の注意**  
ベーシック認証が求められます。別途案内のユーザー名とパスワードを入力してください。

**💬 サポートについて**  
マニュアルに記載の有無に関わらず、疑問点、要望など些細なことでもDiscordにてスタッフまでお問い合わせください。

---

## **目次**

- [全体ワークフロー](#全体フロー)
- [利用開始前の準備（5分）](#利用開始前の準備5分)
  - [1. アカウント作成](#1-アカウント作成)
  - [2. 発信者情報の設定](#2-発信者情報の設定)
- [各機能の使い方](#各機能の使い方)
  - [企画案](#企画案)
  - [冒頭台本](#冒頭台本)
  - [台本全文](#台本全文)
  - [ショート台本](#ショート台本)
  - [ドキュメント編集機能](#ドキュメント編集機能)
- [効果的な使い方](#効果的な使い方)
- [トラブルシューティング](#トラブルシューティング)
- [よくある質問](#よくある質問)

---

## **全体ワークフロー**

ZEROは、台本作成の各フェーズに応じて最適化された専用機能を提供しています。

```mermaid
graph TB
    A[💡 アイデア] --> B[🤖 企画案<br/>AIと対話で具体化]
    B --> C{作成範囲を選択}
    C -->|冒頭のみ| D[✨ 冒頭台本<br/>複数パターン生成]
    C -->|全文| E[📝 台本全文<br/>2段階アウトライン]
    D --> F[📄 ドキュメント編集<br/>手動・AI修正]
    E --> F
    E --> H[🎥 ショート台本<br/>ペルソナ細分化]
    F --> G[🎬 完成台本]
    style A fill:#fff4e6,stroke:#ffa726,stroke-width:2px
    style B fill:#e3f2fd,stroke:#42a5f5,stroke-width:2px
    style C fill:#f3e5f5,stroke:#9c27b0,stroke-width:2px
    style D fill:#e8f5e9,stroke:#66bb6a,stroke-width:2px
    style E fill:#e1f5fe,stroke:#03a9f4,stroke-width:2px
    style F fill:#fce4ec,stroke:#ec407a,stroke-width:2px
    style G fill:#fff3e0,stroke:#ff9800,stroke-width:3px
    style H fill:#fff9c4,stroke:#fbc02d,stroke-width:2px
```

---

## **利用開始前の準備**

### **1. アカウント作成**

<details markdown="1">
<summary>▶ 操作手順を見る</summary>

1. [ツールURL](https://youtube-content-gen.vercel.app/)にアクセス
2. ログイン画面下部の「**新規登録**」をクリック
3. メールアドレスとパスワード（8文字以上）を入力
4. 「**登録**」ボタンで完了（自動ログイン）

| ログイン画面 | 新規登録画面 |
|------------|------------|
| ![ログイン画面](https://github.com/user-attachments/assets/68579087-0cdb-43f2-abda-6837a45c7a66) | ![新規登録画面](https://github.com/user-attachments/assets/ddccc605-72a5-4bed-a1ff-a22269c45797) |

</details>

---

### **2. 発信者情報の設定**

<details markdown="1">
<summary>▶ 操作手順を見る（推奨）</summary>

#### **なぜ必要？**

ZEROがパーソナライズされた台本を作成するためです。

#### **設定方法**

1. 画面左下の**メールアドレス**をクリック
2. 「**マイページ**」ボタンをクリック
3. 以下の項目を入力して「**保存する**」

| 設定画面への入り口 | 設定画面 |
|-----------------|---------|
| ![発信者情報の設定をクリック](https://github.com/user-attachments/assets/eddbf35f-6c5a-437d-9a3b-233e8ab988ea) | ![発信者情報の設定画面](https://github.com/user-attachments/assets/154accd8-8afd-46d7-b6e8-feaf79bdfc14) |

#### **入力項目と例**

| 項目 | 入力例 |
|------|--------|
| **発信者名** | える |
| **一人称** | 僕 |
| **普段の発信内容** | 仕事でも恋愛でも明日から使えるコミュニケーション術 |
| **ターゲット年代** | 20代〜40代 |
| **ターゲット性別** | 男性 |
| **ターゲットの理想** | 仕事：収入を上げて人生をコントロール<br>恋愛：選ぶ側になり素敵なパートナーと出会う |
| **権威性・実績** | ・数千人以上の恋愛相談を解決<br>・書籍「イケメンはモテない」25,000部販売 |

</details>

---

## **各機能の使い方**

### **企画案**

#### **概要**

「このテーマで動画を作りたい」というアイデアを、ZEROとの対話で具体的な企画案にブラッシュアップします。
対話を通してペルソナの詳細分析、独自の切り口の発見を行います。

#### **操作手順**

<details markdown="1">
<summary>▶ 詳しい手順を見る</summary>

1. **起動**: 左メニュー「**企画案**」をクリック
2. **入力**: アイデアをチャットで送信
   - 例：「モテるためのマインドセットについて話したい」
3. **対話**: AIの質問に答える（3〜5回程度）
4. **完成**: 企画案が自動生成される

| 完成イメージ |
|------------|
| ![企画案の対話例](https://github.com/user-attachments/assets/0ec67060-bc45-467b-9be0-1d15a6e10b39) |

#### **実例とデモ**

- [企画作成の実例](https://youtube-content-gen.vercel.app/chat/3e6a4f5b-a42a-42ae-8960-083631875ce7)
- [作成した企画で冒頭を生成](https://youtube-content-gen.vercel.app/chat/07284908-48bc-4942-ad10-50b96914cd0b)

<div style="position: relative; padding-bottom: 56.25%; height: 0;">
<iframe src="https://www.loom.com/embed/bd9d1294dfbd4ac1a0340b2ad105c52b?sid=912e316d-8677-4efe-bd37-b1237a825d63" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen style="position: absolute; top: 0; left: 0; width: 100%; height: 100%;"></iframe>
</div>

</details>

---

### **冒頭台本**

#### **概要**

異なる訴求パターンで最大3つの冒頭台本を生成します。

#### **操作手順**

<details markdown="1">
<summary>▶ 詳しい手順を見る</summary>

##### **STEP 1: 情報入力**

1. 左メニュー「**冒頭台本**」をクリック
2. 必要情報を入力

| 入力項目 | 説明 |
|---------|------|
| **台本または企画案**（必須） | 既存台本または企画案をコピペ |
| **視聴者の悩み**（任意） | 空欄可 |
| **理想の姿**（任意） | 空欄可 |

| 入力画面 |
|---------|
| ![入力フォーム](https://github.com/user-attachments/assets/2cf252bd-9865-4138-ad03-feff487088d7) |

##### **STEP 2: 台本設計の確認**

1. 「**冒頭台本を作成する**」をクリック
2. AIが提示する「台本設計」を確認
3. OKなら「OK」と入力、修正なら指示を入力

| 台本設計画面 | 確認返答画面 |
|------------|------------|
| ![台本設計の確認](https://github.com/user-attachments/assets/4dd9d162-3d7e-460d-bf1e-2cbb7c4b480d) | ![台本設計への返答](https://github.com/user-attachments/assets/cc042166-df29-4fb4-9d6d-b813e42c7ed8) |

##### **STEP 3: パターン選択**

- **A案・B案・C案**から選択
- ボタンクリックで確定、チャット入力で修正可

| 選択画面 |
|---------|
| ![台本案の選択](https://github.com/user-attachments/assets/88ddda63-55b4-4175-845e-b503c7621f01) |

</details>

---

### **台本全文**

#### **概要**

企画案から台本全文をアウトラインを経て生成。

#### **重要コンセプト**

> 💡 **完全自動生成は不可**
>
> **理由**: すべてAIで作ると属人性がなくなり、「再生数は伸びるが売れない」状態を防ぐため
>
> **必須の使い方**:
> 1. 必ず自分自身の考えをAIに投げる
> 2. AIと壁打ちしながら思考をブラッシュアップ
> 3. 構成の合意が取れてから台本生成を開始

#### **操作手順**

<details markdown="1">
<summary>▶ 詳しい手順を見る</summary>

##### **STEP 1: 企画案入力**

1. トップページ「**台本全文**」をクリック
2. 企画案を入力

##### **STEP 2: アウトライン生成**

アウトラインは以下の2つのパートで構成されます。必ず内容を確認してから台本生成を開始してください。

**フェーズ1：戦略設計**
- ターゲット分析と提供価値
- コアアイデア創出
- 全体戦略の選定

**フェーズ2：論理設計**
- 冒頭パートの設計
- 本編パートの設計
- 行動促進パートの設計

##### **STEP 3: 台本生成**

「**✅この内容で確定・台本生成開始**」をクリック（約2-3分）

| アウトライン確認 | 生成結果 |
|----------------|---------|
| ![アウトラインへの合意](https://github.com/user-attachments/assets/bac27063-20ca-48fe-b464-4a15dc54d93c) | ![出力された台本](https://github.com/user-attachments/assets/7e44290f-2fa5-44a2-8867-928a5c6adf7d) |

出力された台本の編集方法は、後述の「[ドキュメント機能](#ドキュメント機能編集バージョン管理)」をご覧ください。

</details>

---

### **ショート台本**

#### **概要**

長尺動画の台本から、ターゲット別のショート動画台本を複数生成。ペルソナをさらに細分化し、それぞれに最適化します。

#### **操作手順**

<details markdown="1">
<summary>▶ 詳しい手順を見る</summary>

1. **起動**: 「**ショート台本**」をクリック
2. **入力**: 長尺台本の全文を貼り付け
3. **分析確認**: ZEROのペルソナ分析を確認し、OKなら「OK」と送信
4. **生成確認**: 複数のショート台本案を取得

</details>

---

### **ドキュメント編集機能**

#### **概要**

生成された台本の編集、推敲、およびバージョン管理を行う機能です。

#### **3つの編集方法**

<details markdown="1">
<summary>▶ 方法1：手動編集</summary>

直接テキストを編集 → 「**保存**」ボタンをクリック

> ⚠️ **注意**: 自動保存機能は未実装です。必ず保存ボタンを押してください。

| 手動編集画面 |
|------------|
| ![台本手入力修正画面](https://github.com/user-attachments/assets/2c8abc42-7433-4ffd-ab4c-5037031e8143) |

</details>

<details markdown="1">
<summary>▶ 方法2：AI編集（部分選択）</summary>

> 💡 **補足**: チャット欄から直接編集指示を送ることも可能です
> ただし、編集範囲を選択しない分、意図しない箇所が編集されるリスクが高いため注意が必要です。
> できるだけ具体的に編集箇所と内容を指示し、必ず差分を確認してから承認してください。

##### **STEP 1: 範囲選択**

編集したい箇所をドラッグ → チャットアイコンをクリック

| 選択前 | 選択後 |
|--------|--------|
| ![image](https://github.com/user-attachments/assets/7dc68942-9d8d-469b-823d-56d5cc40c53c) | ![image](https://github.com/user-attachments/assets/a8495a7e-deed-4b60-89ff-37cb96b608ed) |

##### **STEP 2: 指示入力**

編集指示を入力して送信（例：「もっと簡潔に」）

| 指示入力 | 処理中 |
|---------|--------|
| ![image](https://github.com/user-attachments/assets/ce5bc57b-8287-4cac-bd6c-49a382313f24) | ![image](https://github.com/user-attachments/assets/d5922525-81f7-40c2-b490-eab864ca96ef) |

##### **STEP 3: 差分確認**

「**変更点を表示**」で差分確認

| 確認ボタン | 差分表示 |
|-----------|---------|
| ![image](https://github.com/user-attachments/assets/beb17694-fcb8-48d4-832c-ade183692dfa) | ![image](https://github.com/user-attachments/assets/c09ef247-9820-4baf-b4a9-71b43f1582a6) |

> ⚠️ **注意**: AIは選択範囲の前後も編集する場合があります。必ず差分を確認してください。

##### **STEP 4: 確定/却下**

「**承認**」で反映、「**却下**」で再編集

| 確定画面 |
|---------|
| ![変更確定画面](https://github.com/user-attachments/assets/6dc6ff37-c4bc-4fcf-9c0d-598e63b17f56) |

</details>

<details markdown="1">
<summary>▶ 方法3：バージョン管理</summary>

過去のバージョンに切り替え・復元が可能です。

| バージョン切り替え | バージョン復元 |
|------------------|--------------|
| ![image](https://github.com/user-attachments/assets/ae10d471-eaab-4242-a9c4-961dc4f71541) | ![image](https://github.com/user-attachments/assets/b6887262-e40d-47ef-a1dc-6525ac86aab9) |

</details>

---

#### **⚠️ 現在の制約事項**

**H1見出し（【タイトル・サムネ】【冒頭】【本編】【エンディング】）は編集できません**

- これらの見出しを含む範囲を選択して編集すると、エラーになる場合があります
- AIが誤って見出しを変更しようとした場合もエラーになります
- この制約は開発都合の仕様であり、今後改善予定です

**対処法**: H1見出しを避けて、その下の内容のみを選択して編集してください

---

## **トラブルシューティング**

### **AIの制限事項**

| 制限事項 | 対処法 |
|---------|--------|
| **ハルシネーション** | 公開前に事実確認必須 |
| **最新情報なし** | ネット検索不可、トレンド反映なし |
| **長い会話で精度低下** | 新しいチャットを開始 |
| **完璧ではない** | 指示を変えて再試行 |

---

## **よくある質問**

<details markdown="1">
<summary>▶ Q. ニューロボルトライティングの構成で生成できますか？</summary>

**A.** 固定構成での自動生成は非対応です。

**理由**: テンプレート通りだと毎回同じ構成になり、視聴者が既視感を持つため。

ZEROはテーマに合わせてカスタマイズされた構成を生成します。テンプレートの要素は重視しつつ、柔軟に最適化されます。

</details>

---
